...
/*   Updated: 2025/07/08 13:41:56 by fekandle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_calloc_v2(size_t count, size_t size)
{
	void			*ptr;
	unsigned char	*p;
	size_t			sum;
...
char	*get_next_line(int fd)
{
	...
	data.i = 0;
	data.temp = malloc(1);
	data.buffer2 = malloc(BUFFER_SIZE + 1);
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		...
		buffer = malloc(data.len_buffer + data.len_temp + data.len_buffer2 + 1);
	//
	//
	//
	//
	while (buffer == NULL && data.buffer2 == NULL)
	{
		...
		if (buffer == NULL && ft_strchr(data.buffer2, '\n'))
			buffer = malloc(data.len_temp + data.len_buffer2 + 1);
		//
		//
		while (data.buffer2 != NULL)
		{
			...
			if (data.newline_ptr)
				ft_strlcpy(data.buffer2, data.newline_ptr + 1,
